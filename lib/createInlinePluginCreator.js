...
function createInlinePluginCreator(packages, multiContext) {
	 ...
	 * @internal
	 */
	const _prepare = queuefy(async (pluginOptions, context, pkg) => {
		// Call other plugins.
		await pkg.plugins.prepare(context);
	...
	function createInlinePlugin(pkg) {
		 ...
		 * @internal
		 */
		const analyzeCommits = once(async (pluginOptions, context) => {
		 ...
		 * @internal
		 */
		const generateNotes = once(async (pluginOptions, context) => {
			...
			// await wait(() => todo().every(p => p.hasOwnProperty("_nextRelease")), "_nextRelease");

			// Update pkg deps.
			updateManifestDeps(pkg, path);
			pkg._depsUpdated = true;
			await waitFor("_depsUpdated");
			// await wait(() => todo().every(p => p.hasOwnProperty("_depsUpdated")), "_depsUpdated");
		 ...
		 * @internal
		 */
		const prepare = once(async (pluginOptions, context) => {
			await _prepare(pluginOptions, context, pkg);
			...
			// await wait(() => todo().every(p => p.hasOwnProperty("_prepared")), "_prepared");

			if (packages[0] !== pkg) {
				await ee.once(pkg.name);
			}
		...
		const verifyConditions = once((pluginOptions, context) => plugins.verifyConditions(context));
		const verifyRelease = once((pluginOptions, context) => plugins.verifyRelease(context));
		const publish = once(async (pluginOptions, context) => {
			const result = await plugins.publish(context);

			pkg._published = true;

			const nextToPublish = todo().find(p => !p._published);
			if (nextToPublish) {
		...
		const success = once((pluginOptions, context) => plugins.success(context));
		// istanbul ignore next
		const fail = once((pluginOptions, context) => plugins.fail(context));
