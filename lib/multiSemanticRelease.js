...
async function multiSemanticRelease(
	...
	const blocker = new Promise((resolve) => {
		setInterval(() => {
			const remaining = packages.filter(p => p.isPending).length;
			process.stdout.write(`Jobs remaining: ${remaining}\n`);
	...
	const tasks = packages.map(async (pkg) => {
		...
		// console.log(`${pkg.name}: ${pending}`);
		releasePackage(pkg, createInlinePlugin, multiContext, synchronizer);

		pkg.stderr.pipe(process.stderr);
		pkg.stdout.pipe(process.stdout);
		// console.log(`${pkg.name}: ${pending}`);

		// pkg.result.catch(() => {}).then(() => {
		// 	pending--;
		// 	console.log(`${pkg.name}: ${pending}`);
		// });
		...
		// 	debugger;
		// 	pending--;
		// 	if (pending === 0) baton.resolve();
		// }, () => {
		// 	debugger;
		// 	pending--;
		// 	if (pending === 0) baton.reject();
		// });
		return pkg.result;
	});

	blocker.then(() => {
		debug('blocker resolved');
		debugger;
	}, () => {
		debug('blocker rejected');
		debugger;
	});

	return Promise.all(tasks).then(() => blocker).then((results) => {
		debugger;
 ...
 * @internal
 */
async function releasePackage(pkg, createInlinePlugin, multiContext, synchronizer) {
	// Vars.
	...
	pkg.result = await semanticRelease(options, {
		...
		synchronizer.emit(
			...
			synchronizer.todo().find((p) => p._nextType && !p._prepared)
		);
	...
	// 	baton.pending--;
	// 	if (baton.pending == 0) {
	// 		process.nextTick(() => baton.resolve());
	// 	}
